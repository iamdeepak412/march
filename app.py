...
@app.route('/telegram_n_discord_message', methods=['POST'])
def Telegram_n_discord_message():
    """
    Send a message.
    ---
    parameters:
      - name: platform
        ...
        type: string
        required: true
        description: The platform to send the message to (telegram/discord)
      - name: chat_id
        in: formData
        type: string
        required: true
    ...
    if platform.lower() == 'telegram':
        telegram_url = f"https://api.telegram.org/{TELEGRAM_BOT_TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
        response = requests.post(telegram_url)
        
        if response.status_code == 200:
            return jsonify({'status': 'success', 'message': 'Telegram message sent successfully'})
        else:
        ...
        discord_url = f"https://discord.com/api/v9/channels/{chat_id}/messages"
        response = requests.post(discord_url, json=payload, headers=headers)
        
        if response.status_code == 200:
            return jsonify({'status': 'success', 'message': 'Discord message sent successfully'})
        else:
            return jsonify({'status': 'error', 'message': 'Failed to send Discord message'}), 400
    
    else:
        return jsonify({'status': 'error', 'message': 'Invalid platform specified'}), 400
